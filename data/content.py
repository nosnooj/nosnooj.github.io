        ...
        "career": [
            {
                ...
                "company": "(주)랩지노믹스",
                "role": "선임 · IT팀 · 백엔드 개발",
                "scope": "유전자 분석 LIMS · LIS–ERP 연동 · AWS 인프라 등 사내 핵심 백엔드 시스템의 "
                         "설계 · 구축 · 운영 전반 담당",
                "points": [
                    "유전자 분석 장비 연동 Microarray LIMS(GAIA) 신규 구축 — 대용량 실험 데이터 수집·이관 "
                    "자동화로 데이터 동기화 오류 현재까지 0건",
                    "GAIA 결과지 생성·검사 모니터링 자동화 — 결과지 다운로드 시간 추정 95% 단축",
                    "LIS–ERP10 매출 연동 플랫폼(LESS) 구축 — 수작업 월 결산 처리 약 70% 단축",
                    "온프레미스 → AWS 아키텍처 전환 주도 — 인프라 비용 46% 절감, 가용성 99.9% 이상 확보",
                    "주문·정산 백오피스(COS) 구축 및 운영 웹·사내 LIMS 전반 유지보수 — 운영 이슈 100여 건 대응",
                ],
            },
            {
                ...
                "scope": "B2C 헬스케어 서비스의 외부 데이터 API 연동 · 결제 · 백오피스 · 리포트 자동화 백엔드 개발",
                "points": [
                    "PHR·유전자 데이터 기반 B2C 영양제 추천·보험 상담 연계 플랫폼(IB) 구축 — 개인화 추천 "
                    "알고리즘 및 KCP 결제 연동",
                    "ClipSoft Report 기반 결과지 자동화 — 양식 변경을 코드 배포 없이 반영하도록 "
                    "템플릿·데이터 매핑 계층 분리",
                ],
            },
            {
                ...
                "scope": "미국 애드테크 기업의 광고·정산 데이터 운영 자동화 및 대용량 데이터 분석 (한국 원격 근무)",
                "points": [
                    "수백만 건 광고 데이터의 익월 예산 산정·100여 개 계정 Invoice 생성을 "
                    "Excel·VBA + Python 크롤링·캡처로 반자동화 — 반복 업무 시간 대폭 단축",
                    "광고 플랫폼 운영 이슈를 Oracle SQL 대용량 데이터 분석으로 원인 규명·해결",
                ],
            },
        ],
        "skills": [
...
PROJECTS = [
    {
        ...
        "build": [
            {"module": "장비 상태 실시간 수집 · 대용량 결과 이관",
             "points": [
                 "MQTT(Subscribe/Publish)로 장비 Run 상태(진행률·오류)를 실시간 구독·수집해 모니터링에 반영",
                 "Run 종료 시 대용량 결과 파일을 Socket 통신(TCP Socket·WebSocket)으로 분석 서버에 전송 "
                 "— 폐쇄망 고속 전송 활용, CompletableFuture 병렬 전송으로 다수 파일을 동시 처리해 이관 시간을 평균 25~30분에서 약 5분으로 단축",
                 "이관 과정의 중복·충돌·경로 오류를 유형별로 방어해 운영 후 현재까지 이관 오류 0건",
             ]},
            {"module": "분석 파이프라인 작업 오케스트레이션",
             "points": [
            ...
            {"module": "결과 판독 웹 · 수진자 매핑 · LIS 양방향 연동",
             "points": [
                 ...
                 "판독 완료 결과를 LIS로 연동하는 데몬을 구축해, 장비에서 판독을 거쳐 LIS까지 이어지는 "
                 "흐름을 양방향으로 자동 연계, 시스템 간 이중 입력·수작업 매핑 제거",
             ]},
            {"module": "결과지 자동 생성 데몬 · 다운로드 · 검사 모니터링",
             "points": [
                 "Crownix Report(ClipSoft)로 최종 처리된 샘플 결과지를 매시간 자동 생성하는 스케줄 데몬 구축",
                 "수진자 정보 변경·결과 재입력 시 API 호출로 해당 결과지만 선별 재생성",
                 "기존 LIS는 결과지를 건별로만 내려받을 수 있던 것을 일괄(다건) 다운로드로 확장하고 "
        ...
        "result": {
            ...
            "retro": "운영에서 사람 손이 닿을 지점을 개발 전에 미리 과제로 정의해 둔 덕에, "
    ...
    },
    {
        ...
        "card_one_liner": "검사–회계 데이터 흐름 연동 — 월 결산 약 70% 단축",
        "card_stack": ["Java", "Spring", "WebFlux", "ERP 연동"],
        "hero": {
            "kicker": "CASE STUDY · CS2 · 2024.04–2024.10",
            "title": "검사 매출에서 ERP까지, 손으로 맞추지 않는 결산",
        ...
        "glance": {
            ...
            "key_result": "수작업 월 결산을 자동화, 처리 시간 약 70% 단축",
        },
        "problem": (
        ...
        "decisions": [
            {"head": "검사·매출 분야별로 정산을 전략으로 나눈다",
             "body": "분야마다 다른 조회·정산 규칙을 하나의 분기문에 몰지 않고, BillingService 아래 "
                     "분야별 구현체(전략 패턴)로 분리했습니다. 팀원별 분담 개발과 새 매출 분야 확장을 "
                     "기존 코드 충돌 없이 가능하게 합니다."},
            {"head": "대량 연동은 WebFlux 논블로킹으로 일괄 처리한다",
             "body": "결산 시점에 매출이 한꺼번에 몰리므로, Spring WebFlux·WebClient 논블로킹으로 "
                     "수주(SO)·생산지시를 일괄 전송합니다. 건마다 응답을 기다리는 블로킹 방식 대비 "
                     "대기 시간을 줄였습니다."},
            {"head": "원천과 운영 데이터를 읽기/쓰기로 분리한다",
             "body": "원천인 LIS(MSSQL)는 읽기 전용, 가공·연동 데이터는 운영 DB(MySQL)에 쓰도록 "
                     "MyBatis SqlSessionFactory를 데이터소스별로 분리해 원천 데이터 오염 위험을 "
                     "차단했습니다."},
            {"head": "연동을 건별 상태로 추적해 실패를 격리한다",
             "body": "전송을 건별 성공/실패 상태로 기록하고, 실패한 건만 선별해 재연동할 수 있게 "
                     "했습니다. 일부 실패가 전체 마감을 막지 않도록 격리해 월 결산의 신뢰성을 "
                     "확보했습니다."},
        ],
        "build": [
            {"module": "매출 데이터 취합 · 조회 · 검증",
             "points": [
                 "실무자 정산 프로세스에 맞춰 자사 LIS(MS SQL Server)에서 매출을 취합·조회·검증하는 "
                 "웹 화면을 백엔드부터 프런트엔드(Thymeleaf·jQuery·TUI Grid)까지 구현",
                 "검사·매출 분야별로 조회·정산 로직을 분리하는 전략 패턴 위에서 담당 분야 구현체를 "
                 "개발 — 분야별 분담 개발과 신규 분야 확장이 용이",
                 "무료조정금액 검증과 중복 방지(해당 월 데이터 삭제 후 재등록)로 결산 데이터 정확도 확보",
             ]},
            {"module": "ERP10 연동 (수주 · 생산지시)",
             "points": [
                 "검증된 매출을 ERP10 인터페이스로 전송 — LIS 독자 규격(CamelCase)을 ERP10 "
                 "규격(Snake_case)으로 변환하는 매핑 계층 적용",
                 "Spring WebFlux·WebClient 논블로킹으로 수주(SO)·생산지시 정보를 일괄 전송 "
                 "— 대량 매출 처리 시 블로킹 대비 지연 최소화",
                 "RSA 암호화 토큰 인증·운영 환경 Proxy 연동으로 ERP10 API 보안 연동",
             ]},
            {"module": "연동 이력 관리 · 재처리",
             "points": [
                 "건별 연동 상태(성공/실패)와 에러 로그를 추적·저장하고, 실패 건만 선별 재연동하는 기능 구축",
                 "결산 매출 데이터를 Excel(Apache POI)로 추출",
             ]},
            {"module": "공통 기반",
             "points": [
                 "다중 데이터소스 — 읽기 전용 LIS(MSSQL)와 쓰기 운영 DB(MySQL)를 MyBatis SqlSessionFactory로 분리",
                 "Spring Security 인증 · Argon2 해싱 · 역할 기반 접근제어",
                 "운영 환경 Redis 세션 · HikariCP 커넥션 풀",
             ]},
        ],
        "result": {
            "metrics": [
                {"value": "약 70%↓", "label": "월 결산 처리 시간 단축 (수일→수시간, 실무자 평가)"},
                {"value": "수작업 → 자동", "label": "월 매출 정산 수작업 제거 — 실무진은 검증에 집중"},
            ],
            "retro": "검사 분야마다 다른 정산 규칙을 전략으로 나누고 실패 건을 격리해 둔 덕에, "
                     "며칠씩 손으로 맞추던 월 결산을 시스템이 대신 돌리고 실무진은 검증에 집중하게 됐습니다.",
        },
    },
    {
        "id": "cs3", "slug": "aws", "code": "CS3", "accent": "amber",
        "name": "온프레미스 → AWS 아키텍처 전환",
        "period": "2024.01 ~ 2024.06",
        "card_one_liner": "단일 서버에서 다중 AZ 이중화로 — 비용 46%↓, 가용성 99.9%↑",
        "card_stack": ["AWS VPC", "Nginx ARR", "Podman", "NFS·Redis"],
        "hero": {
            "kicker": "CASE STUDY · CS3 · 2024.01–2024.06",
            "title": "한 대가 멈춰도, 멈추지 않는 인프라",
        ...
        "glance": {
            ...
            "key_result": "인프라 비용 약 46% 절감, 가용성 99.9% 이상",
        },
        "problem": (
        ...
        "decisions": [
            {"head": "퍼블릭/프라이빗을 다중 AZ로 나눈다",
             "body": "Nginx·Bastion만 퍼블릭 서브넷에 두고 WAS·DB·NFS·Redis는 프라이빗 서브넷으로 "
                     "내려, 운영 자원의 외부 노출을 없앴습니다. 서브넷을 여러 가용영역(AZ)에 분산해 "
                     "한 영역 장애가 전체로 번지지 않게 했습니다."},
            {"head": "리버스 프록시를 이중화해 단일 장애점을 없앤다",
             "body": "기존에는 한 대의 호스트가 리버스 프록시(ARR)를 전담해 단일 장애점이었습니다. "
                     "NLB 뒤에 Nginx ARR 노드를 두 대로 이중화하고 HSTS를 적용해, 한 대가 멈춰도 "
                     "트래픽이 다른 노드로 흐르게 했습니다."},
            {"head": "이중화된 설정은 자동 동기화로 일치시킨다",
             "body": "이중화한 두 Nginx 노드의 설정이 어긋나면 그대로 장애가 되므로, rsync로 설정을 "
                     "노드 간 자동 동기화해 구성 불일치를 차단했습니다."},
            {"head": "상태·파일을 분리해 무상태 웹을 만든다",
             "body": "세션은 Redis로, 이미지·업로드 같은 공통 파일은 저비용 EC2·EBS 기반 자체 NFS로 "
        ...
        "build": [
            {"module": "VPC 네트워크 · 보안",
             "points": [
                 "다중 AZ에 퍼블릭(Nginx·Bastion)·프라이빗(WAS·DB·NFS·Redis) 서브넷 분리 설계",
                 "Bastion Host 단일 SSH 경로로 접근 통제 — 온프레미스 IP만 허용, 내부·외부 경로 분리",
                 "NAT Gateway로 프라이빗 아웃바운드 분리, Internet Gateway는 퍼블릭 계층에만 연결",
             ]},
            {"module": "로드밸런싱 · 이중화 (가용성)",
             "points": [
                 "NLB 뒤 Nginx 리버스 프록시(ARR) 2노드 이중화 · HSTS 적용 — 단일 리버스 프록시(AS-IS)의 단일 장애점 제거(TO-BE)",
                 "웹 호스팅 노드(Podman 컨테이너)와 Java WAS를 다중 AZ에 분산·이중화",
                 "rsync로 Nginx 설정을 노드 간 자동 동기화해 구성 불일치 방지",
             ]},
            {"module": "세션 · 공유 스토리지 · DB",
             "points": [
                 "Redis 세션 스토어로 다중 웹 노드 간 세션 공유(무상태 웹 계층)",
                 "저비용 EC2·EBS 기반 자체 NFS로 이미지·업로드 파일 등 공통 파일을 전 노드에서 동일 접근",
                 "MariaDB 프라이빗 서브넷 전용 인스턴스 운영",
             ]},
            {"module": "전환",
             "points": [
                 "온프레미스 서비스를 무중단 기준으로 단계적 이관, 자원 구성 재설계로 비용 약 46% 절감",
                 "단계적 전환·검증을 반복하며 가용성 99.9% 이상으로 안정화",
             ]},
        ],
        "result": {
            "metrics": [
                {"value": "약 46%↓", "label": "인프라 비용 절감"},
                {"value": "99.9%↑", "label": "시스템 가용성 (단일 장애점 제거)"},
                {"value": "무중단 이관", "label": "온프레미스 서비스 단계적 전환"},
                {"value": "무상태 웹", "label": "세션·파일 분리로 수평 확장"},
            ],
            "retro": "한 대가 멈춰도 서비스가 멈추지 않도록 이중화와 상태 분리를 먼저 설계한 덕에, "
    ...
    },
    {
        ...
        "card_one_liner": "엑셀 수작업 정산을 단일 백오피스로 — 잔액 검증·감사 추적",
        "card_stack": ["Java", "Spring", "PI 선불계정", "정산·회계"],
        "hero": {
            "kicker": "CASE STUDY · CS4 · 2024.03–2024.06",
            "title": "주문에서 정산까지, 엑셀을 떠난 한 화면",
        ...
        "glance": {
            ...
            "key_result": "엑셀 수작업 정산을 단일 백오피스로 일원화 · 모든 변경 감사 추적",
        },
        "problem": (
        ...
        "decisions": [
            {"head": "거래처를 기관–PI–고객 마스터로 표준화한다",
             "body": "주문·정산의 기준이 되는 거래처 정보를 기관·PI·고객 마스터로 정규화하고 소속 "
                     "관계로 연결해, 엑셀마다 다르던 거래처 표기를 한 곳으로 모았습니다."},
            {"head": "PI 선불계정 잔액을 주문의 관문으로 둔다",
             "body": "주문 등록 시 PI 선불계정 잔액과 주문총액을 비교해, 잔액이 부족하면 경고로 "
                     "막았습니다. 회계기관계정에서 PI 계정으로 금액을 이동(충전)하는 경로와 PI 계정 간 "
                     "이체를 구분하고, 잔액·미수금을 함께 관리해 수기 계산 오류가 정산에 흘러드는 길을 "
                     "끊었습니다."},
            {"head": "결제·입금은 취소·환불까지 정합을 맞춘다",
             "body": "결제·입금의 등록뿐 아니라 취소·환불까지 다루되, 취소·환불 시 회계기관계정의 "
                     "결제누적총액·입금총액을 되돌려 재정합했습니다. 정산 데이터가 어느 시점에도 맞도록 했습니다."},
            {"head": "모든 변경을 감사 추적으로 남긴다",
             "body": "주문·정산은 돈이 걸린 업무라, 사용자 행위를 Audit·로깅으로 남기고 권한 기반 "
        ...
        "build": [
            {"module": "고객 · 기관 · PI 관리",
             "points": [
                 "연구기관(기관 및 소속)·PI·고객 정보를 등록·조회·수정하는 거래처 마스터 관리 구현",
                 "기관–PI–고객 소속 관계를 연계해 주문·정산의 기준 데이터로 활용",
             ]},
            {"module": "견적 · 주문 · 샘플",
             "points": [
                 "견적서 등록·조회·인쇄와 주문 등록(기본정보·샘플정보·주문회계)을 연계한 주문 프로세스 구현",
                 "유전체 검사 샘플의 수집일자·반환 여부 등 상태를 주문 단위로 관리",
             ]},
            {"module": "PI 선불계정 정산 · 회계",
             "points": [
                 "회계기관계정에서 PI 선불계정으로 금액 이동(충전), PI 계정 간 이체는 별도 경로로 처리하고 잔액 초과를 검증",
                 "주문 시 주문총액과 PI 잔액 비교(잔액 부족 시 경고), 잔액·미수금 관리",
                 "결제·입금 등록/취소/환불 연동, 취소·환불 시 회계기관계정 결제누적·입금총액 재정합",
             ]},
            {"module": "공통 기반",
             "points": [
                 "사용자 행위 Audit·로깅 이력 관리",
                 "권한 기반 접근 제어",
                 "페이징·검색 표준 화면",
             ]},
        ],
        "result": {
            "metrics": [
                {"value": "엑셀 → 단일 시스템", "label": "흩어진 주문·정산 업무 일원화"},
                {"value": "전 변경 감사 추적", "label": "주문·정산 변경 이력 기록·추적"},
            ],
            "retro": "선불계정 잔액 검증과 감사 추적을 시스템의 기본값으로 만든 덕에, 엑셀로 "
    ...
    },
    {
        ...
        "glance": {
            ...
            "period": "2022.08 – 2023.07",
            "stack": ["PHP", "JavaScript", "MySQL", "Apache", "KCP 결제", "ClipSoft",
                      "Open API", "알림톡", "Docker"],
            "key_result": "설문·건강검진 결합 영양제 추천부터 결제·리포트 자동화까지 구축",
        },
        "problem": (
        ...
        "decisions": [
            {"head": "추천은 설문과 건강검진 결과를 결합해 만든다",
             "body": "상품 접수 단계에서 받은 고객 설문 답변과 건강검진 결과(PHR)를 함께 반영해, "
                     "개인별 영양제를 추천하는 알고리즘을 구성했습니다."},
            {"head": "흩어진 건강 데이터는 외부 API로 모은다",
             "body": "건강검진 결과, 협력사의 신체나이 분석, 질병 위험도를 예측하는 유전자 검사 결과를 "
                     "Open API·협력사 API로 실시간 연동했습니다. 영양제 상품 목록은 협력사에서 받아 "
                     "DB로 관리했습니다."},
            {"head": "민감 정보는 동의 범위 안에서만 다룬다",
             "body": "건강검진·유전 정보 같은 민감 데이터를 고객이 동의한 범위에서만 수집·전달하도록 "
                     "접근 통제와 처리 경로를 설계했습니다. 보험도 직접 추천하지 않고, 동의받은 결과만 "
                     "상담자에게 전달하는 연계로 풀었습니다."},
            {"head": "리포트 양식 변경을 운영에서 흡수한다",
             "body": "ClipSoft 기반 리포트에서 양식을 코드 배포 없이 바꿀 수 있도록 템플릿과 데이터 "
                     "매핑 계층을 분리했습니다. 잦은 양식 변경이 개발 일정에 묶이지 않게 했습니다."},
        ],
        "build": [
            {"module": "개인화 추천 · 데이터 연동",
             "points": [
                 "건강검진 결과(PHR)·신체나이 분석·유전자 검사 결과를 Open API·협력사 API로 실시간 수집",
                 "상품 접수 단계의 고객 설문 답변과 건강검진 결과를 결합한 개인화 영양제 추천 알고리즘 구현",
                 "협력사에서 전달받은 영양제 상품 목록을 DB에 적재·관리해 추천 결과에 맞춰 노출",
                 "보험은 직접 추천 대신, 동의받은 건강검진·신체나이·유전자 결과를 보험 상담자에게 제공하는 상담 연계로 설계",
             ]},
            {"module": "결제 시스템",
             "points": [
                 "KCP 결제 모듈 연동으로 주문–결제 프로세스 구축",
                 "결제 예외·검증 처리로 결제 안정성 확보",
             ]},
            {"module": "건강 리포트 자동화",
             "points": [
                 "ClipSoft 기반 개인 맞춤 건강 리포트 자동 생성",
                 "양식 변경을 코드 배포 없이 반영하도록 템플릿·데이터 매핑 계층 분리",
                 "알림톡 연계로 리포트 발송·고객 커뮤니케이션 자동화",
             ]},
            {"module": "관리자 백오피스",
             "points": [
                 "고객 관리·주문 관리·데이터 분석 인터페이스를 갖춘 통합 운영 백오피스 구축",
             ]},
        ],
        "result": {
            "metrics": [
                {"value": "추천→결제→리포트", "label": "한 흐름으로 자동화한 B2C 서비스"},
                {"value": "양식 무배포 변경", "label": "ClipSoft 템플릿·데이터 매핑 분리"},
            ],
            "retro": "추천부터 결제·리포트까지 한 흐름으로 이어 두고 양식 변경을 운영에서 흡수하게 "
